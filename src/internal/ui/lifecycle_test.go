// These tests rely on testing/synctest (fake clock plus deterministic goroutine
// scheduling), which has no Go 1.20 equivalent. This fork builds with Go 1.20,
// the last release supporting macOS 10.13, so the file is gated to toolchains
// that provide synctest.Test and still runs unchanged upstream.
//go:build go1.25
